// Background service worker: owns the "open Kanbkam, paste the link into
...
function runKanbkamPasteSearch(pasteValue, fallbackUrl) {
  chrome.tabs.create({ url: KANBKAM_HOME, active: true }, (tab) => {
...
function attemptFill(tabId, pasteValue, fallbackUrl) {
  chrome.scripting.executeScript(
    ...
    (results) => {
      const ok = results && results[0] && results[0].result && results[0].result.ok;
      if (!ok && fallbackUrl) {
